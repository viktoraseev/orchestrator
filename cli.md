...
1. Явно переданный WorkflowId всегда выбирается первым; `config.yaml` и список
   файлов для выбора не читаются.
...
`config.yaml`, если существует и требуется для выбора, должен быть YAML mapping
с единственным необязательным полем `default-workflow`. Значение обязано быть
валидным WorkflowId. Невалидный config завершается с `3`. Если default ссылается
на отсутствующий workflow, команда завершается с `4` и не переключается на
единственный или другой доступный workflow.

## `config get`, `config set` и `config list`

Сейчас поддерживается единственный ключ `default-workflow`.

- `orchestrator config get default-workflow` печатает WorkflowId без имени ключа
  или `null`, если default не задан.
- `orchestrator config list` печатает
  `default-workflow: <workflow-id>` либо `default-workflow: null`. При добавлении
  настроек каждая печатается на отдельной строке в фиксированном порядке. Команда
  не перечисляет доступные workflow-файлы.
...
Отсутствующий `config.yaml` эквивалентен unset default: `get` и `list`
завершаются с `0`. Неизвестный ключ или неверное число аргументов завершается с
`2`. Невалидный существующий config завершается с `3` и не перезаписывается.
Отсутствующий workflow для `set` завершается с `4` без изменения config. Config
commands не получают run locks и не изменяют существующие runs; конкурентные
успешные `set` публикуют целый файл атомарно, и остаётся значение последнего
durable commit.
