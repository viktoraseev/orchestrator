Feature: Публикация Agent completion и artifacts
  ...
  Rule: Attempt complete принимает полный snapshot объявленных outputs
    ...
    Scenario: Бинарный artifact публикуется под точным durable-именем без output marker
      ...
      Then lifecycle завершается с кодом 0
      And единственный durable artifact является regular file 0.first.result.artifact с исходными binary bytes
      And отдельный output.yaml не создан
      And attempt завершён terminal event completed
